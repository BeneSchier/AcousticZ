...
class Room:
    ...
    def createReceiver(self, receiver=np.array([5, 2, 1.3]), radiusOfReceiverSphere=1.0):
...
# 
    ...
    def performRayTracing_vectorized(self):
        ...
        scene = trimesh.Scene([self.room, self.sourceCoord, self.receiverSphere])
        scene.show()
        for iBand in tqdm(range(6)):
            ...
            while (np.any(ray_time <= self.imResTime) and no_hit_counter < 100):
                ...
                paths = np.vstack((paths, np.hstack((ray_xyz_corr, target)).reshape(-1, 2, 3)))
                
                ray_visualize_scene.add_geometry(trimesh.load_path(np.hstack((ray_xyz_corr, target)).reshape(-1, 2, 3), width=0.001))
                
                # ray_visualize_scene.add_geometry(trimesh.load_path(np.hstack((ray_xyz, self.room.face_normals[indexOfFace])).reshape(-1, 2, 3), width=0.001))
                ...
                if (np.any(np.linalg.norm(target - ray_xyz, axis=1) < 1e-10)):
                    ...
                    error_index = np.where(np.linalg.norm(target - ray_xyz, axis=1) < 1e-06)[0]
                    error_plot1 = trimesh.Scene()
                    error_plot1 = (trimesh.load_path(np.hstack((ray_xyz[error_index], ray_xyz[error_index] + ray_dxyz[error_index])).reshape(-1, 2, 3), width=0.001))
                    error_plot2 = (trimesh.load_path(np.hstack((ray_xyz[error_index], ray_xyz[error_index] + self.room.face_normals[indexOfFace_old[error_index]])).reshape(-1, 2, 3), width=0.001))
                    
                    error_scene = trimesh.Scene([error_plot1, error_plot2, self.room])
                    # error_scene.show()
                ...
                if(ray_xyz.size == 0):
                    print('No more ray hits found')
                    break
                # Initialize another intersector to determine all the rays that hit the receiver
...
# 
        ...
        scene = trimesh.Scene([self.room, ray_visualize_scene, self.receiverSphere])
        scene.show()
    ...
    def generateRoomImpulseResponse(self):
        ...
        # V = 1.0
        print('Volume = ', V)
        print('Volume of boundary Box', self.getRoomVolume())
        c = 343.0
        ...
        for index in range(numFrames):
            ...
            # Compute STFT
            # _, _, X = scipy.signal.stft(x, fs=fs, window=win, nperseg=882, noverlap=441, nfft=NFFT, boundary='even')
            _, _, X = scipy.signal.stft(x, fs=fs, nfft=NFFT)
            
            X = X[:, 1:7] * RCF.T
            # X = X * RCF.reshape(-1, 1)
            # Compute inverse STFT
            # _, y_frame = scipy.signal.istft(X, fs=fs, window=win, nperseg=882, noverlap=441, nfft=NFFT, boundary='even')
            _, y_frame = scipy.signal.istft(X, fs=fs, nfft=NFFT)
            ...
            # X = X * RCF
            # # y[(index * 8192):((index + 1) * 8192), :] = (np.fft.irfft(X, NFFT)[:frameLength, :]).T
            # y = (np.fft.irfft(X, NFFT)[:frameLength, :]).T


        # Combine the filtered sequences
        impTimes = (1 / fs) * np.arange(y.shape[0])
        hisTimes = self.histTimeStep / 2 + self.histTimeStep * np.arange(self.nTBins)
        W = np.zeros((y.shape[0], len(FVect)))
        BW = fhigh - flow
        #print('check for greater than zero: ', BW)
        ...
        for k in range(self.TFHist.shape[0]-1):
            ...
            #print('check for greater than zero: ', np.sum(yy, axis=0))
            #print('check for greater than zero: ', self.TFHist[k, :])
            # print('yy = ', yy)
            # print('check for zero: ', np.sum(yy, axis=0))
            # print('check for zero: ', np.sum(yy, axis=0))
            
            val = np.sqrt(self.TFHist[k, :] / np.sum(yy, axis=0)) * np.sqrt(BW / (fs / 2))
            if (np.any(np.isnan(val))):
                print('on top: ', np.sqrt(self.TFHist[k, :]))
                print('below', np.sum(yy, axis=0))
            W[gk0:gk1, :] = val
    ...
    def generateRIR(self):
        ...
        for index0 in range(len(flow)):
            for index in range(len(F)):
                ...
                if f < flow[index0] and f >= flow[index0] - 10:
                    RCF[index0, index] = 0.5 * (1 + np.cos(2 * np.pi * f / flow[index0]))
                elif f < fhigh[index0] and f >= fhigh[index0] - 10:
                    RCF[index0, index] = 0.5 * (1 - np.cos(2 * np.pi * f / fhigh[index0]))
        ...
        envelope = np.random.randn(len(y))  # Simulated energy envelope (replace with actual envelope)
...
if __name__ == '__main__':
    ...
    #r.generateRIR()
    r.applyRIR(calculate_RIR(r.TFHist, 1, 1.0, 44100))
    #r.performRayTracing()
